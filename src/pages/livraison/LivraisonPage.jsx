...
function Card({ children, style }) {
  return <div style={{ background:'white', borderRadius:8, border:'1px solid #E5E7EB', ...style }}>{children}</div>
}

const TABS_CLIENT = ['Envoyer un colis', 'Suivre un colis', 'Mes envois']
const TABS_VISITEUR = ['Envoyer un colis', 'Suivre un colis']
...
const inputStyle = { width:'100%', padding:'0.75rem 0.9rem', border:'1px solid #D1D5DB', borderRadius:6, fontSize:'0.88rem', outline:'none', fontFamily:'system-ui,sans-serif', color:'#111827' }

export default function LivraisonPage() {
  const navigate = useNavigate()
  const user = getUser()
  const connecte = isAuthenticated()
  const TABS = connecte ? TABS_CLIENT : TABS_VISITEUR
  ...
  useEffect(() => {
    if (tab === 'Mes envois' && connecte) chargerColis()
  }, [tab])
  ...
  const handleEnvoyer = async () => {
    if (!connecte) {
      navigate('/auth/login?redirect=/livraison')
      return
    }
  ...
  const handleSuivi = async () => {
    if (!otpInput || otpInput.length !== 6) {
      setOtpError('Entrez un code OTP de 6 chiffres')
      return
    }
    setOtpError('')
    setLoading(true)
      ...
      <div style={{ marginTop:108, background:'#F9FAFB', minHeight:'100vh' }}>
        ...
        <div style={{ background:'#C2410C', padding:'1.8rem 3rem' }}>
          <div style={{ maxWidth:1000, margin:'0 auto' }}>
            ...
            <p style={{ color:'rgba(255,255,255,0.7)', fontSize:'0.85rem', fontFamily:'system-ui,sans-serif' }}>
        ...
        <div style={{ background:'white', borderBottom:'1px solid #E5E7EB' }}>
          <div style={{ maxWidth:1000, margin:'0 auto', padding:'0 3rem', display:'flex' }}>
            {TABS.map(t => (
        ...
        <div style={{ maxWidth:1000, margin:'0 auto', padding:'2rem 3rem' }}>
            ...
            <div>
              ...
              {step === 1 && (
                <Card style={{ padding:'1.8rem' }}>
                  <div style={{ fontWeight:600, fontSize:'1rem', color:'#111827', marginBottom:'1.3rem', fontFamily:'system-ui,sans-serif' }}>
                    Informations sur le colis
                  </div>

                  {!connecte && (
                    <div style={{ background:'#FEF9C3', border:'1px solid #FDE68A', borderRadius:6, padding:'0.8rem 1rem', marginBottom:'1rem', fontSize:'0.82rem', color:'#713F12', fontFamily:'system-ui,sans-serif' }}>
                      Vous devez être connecté pour envoyer un colis.
                      <button onClick={() => navigate('/auth/login?redirect=/livraison')}
                        style={{ color:'#C2410C', fontWeight:700, background:'none', border:'none', cursor:'pointer', marginLeft:6, fontFamily:'system-ui,sans-serif', fontSize:'0.82rem' }}>
                        Se connecter
                      </button>
                    </div>
                  )}
                  ...
                  <div style={{ background:'#FEF9C3', border:'1px solid #FDE68A', borderRadius:6, padding:'0.8rem 1rem', marginBottom:'1.2rem', fontSize:'0.8rem', color:'#713F12', fontFamily:'system-ui,sans-serif' }}>
                    Un code OTP unique sera généré. Le conducteur ne peut valider la livraison qu'avec ce code communiqué par le destinataire.
                  </div>

                  <button onClick={() => { if (!connecte) { navigate('/auth/login?redirect=/livraison'); return; } setError(''); setStep(2) }}
              ...
              {step === 3 && colisCreé && (
                <Card style={{ padding:'2rem', textAlign:'center' }}>
                  ...
                  <div style={{ background:'#FEF9C3', border:'1px solid #FDE68A', borderRadius:6, padding:'0.8rem 1rem', marginBottom:'1.5rem', fontSize:'0.8rem', color:'#713F12', fontFamily:'system-ui,sans-serif' }}>
                    Communiquez ce code uniquement au destinataire <strong>{colisCreé.nomDestinataire}</strong>. Le conducteur devra saisir ce code pour confirmer la livraison.
                  </div>
              ...
              {otpResult && otpResult !== 'non trouvé' && (
                <div style={{ background:'#F9FAFB', borderRadius:8, padding:'1.2rem', border:'1px solid #E5E7EB' }}>
                  ...
                  {[
                    ...
                    ['Poids', `${otpResult.poids} kg`],
                    ['Destinataire', otpResult.nomDestinataire],
                    ...(otpResult.nomConducteur ? [['Conducteur', otpResult.nomConducteur]] : []),
                  ].map(([label, val]) => (
                    <div key={label} style={{ display:'flex', justifyContent:'space-between', padding:'0.5rem 0', borderBottom:'1px solid #F3F4F6' }}>
                      <span style={{ fontSize:'0.82rem', color:'#6B7280', fontFamily:'system-ui,sans-serif' }}>{label}</span>
                      <span style={{ fontSize:'0.82rem', color:'#111827', fontWeight:600, fontFamily:'system-ui,sans-serif' }}>{val}</span>
