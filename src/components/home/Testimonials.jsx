...
export default function Testimonials() {
  return (
    <div style={{ background:'var(--gray)', padding:'5rem 2rem' }}>
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <div style={{ marginBottom:'2.5rem' }}>
          ...
          <p style={{ color:'var(--muted)', fontSize:'0.9rem', marginTop:'0.5rem', fontFamily:'system-ui,sans-serif' }}>
            Ce que disent nos utilisateurs de Béni Mellal et d'ailleurs
          </p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(300px, 1fr))', gap:'1.5rem' }}>
          {reviews.map(r => (
            <div key={r.name}
              style={{ background:'white', borderRadius:16, padding:'1.8rem', border:'1px solid var(--border)', transition:'all 0.2s' }}
              ...
              <p style={{ color:'#374151', fontSize:'0.92rem', lineHeight:1.72, marginBottom:'1.3rem', fontStyle:'italic', fontFamily:'system-ui,sans-serif' }}>"{r.text}"</p>
              <div style={{ display:'flex', alignItems:'center', gap:'0.8rem' }}>
                <div style={{ width:42, height:42, borderRadius:'50%', background:'var(--primary-light)', display:'flex', alignItems:'center', justifyContent:'center', fontWeight:700, fontSize:'0.85rem', color:'var(--primary)', flexShrink:0 }}>{r.initials}</div>
                <div>
                  <div style={{ fontWeight:600, fontSize:'0.88rem', color:'var(--dark)', fontFamily:'system-ui,sans-serif' }}>{r.name}</div>
                  <div style={{ fontSize:'0.77rem', color:'var(--muted)', fontFamily:'system-ui,sans-serif' }}>{r.role}</div>
