...
const services = [
  {
    ico:'🚗',
    title:'Covoiturage longue distance',
    desc:'Partagez vos trajets interurbains et divisez les frais. Béni Mellal ↔ Casablanca, Marrakech ↔ Rabat...',
    bg:'rgba(0,135,90,0.15)',
    lien:'/trajets',
    label:'Voir les trajets'
  },
  {
    ico:'📦',
    title:'Livraison sécurisée OTP',
    desc:'Envoyez vos colis avec un conducteur en déplacement. Validation par code OTP unique à la livraison.',
  ...
  {
    ico:'💬',
    title:'Négociation de tarif',
    desc:'Proposez votre prix, le conducteur accepte ou contre-propose. Le bon prix pour les deux parties.',
  ...
  },
  {
    ...
    desc:"Chaque conducteur soumet ses documents d'identité. Notre équipe les vérifie avant toute activation.",
...
export default function Services() {
  return (
    <div style={{ background:'var(--dark)', padding:'5rem 2rem' }}>
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <div style={{ marginBottom:'2.5rem' }}>
          ...
          <h2 style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:'clamp(1.8rem, 4vw, 2.6rem)', fontWeight:800, color:'white', marginBottom:'0.8rem' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(250px, 1fr))', gap:1, background:'rgba(255,255,255,0.08)', borderRadius:16, overflow:'hidden' }}>
          {services.map(s => (
            <div key={s.title}
              style={{ background:'var(--dark2)', padding:'2.2rem', transition:'background 0.2s' }}
              ...
              <p style={{ color:'rgba(255,255,255,0.5)', fontSize:'0.87rem', lineHeight:1.65, marginBottom:'1rem', fontFamily:'system-ui,sans-serif' }}>{s.desc}</p>
              {/* ✅ Lien fonctionnel */}
              <Link to={s.lien}
                style={{ color:'var(--gold)', fontSize:'0.82rem', fontWeight:600, textDecoration:'none', display:'inline-flex', alignItems:'center', gap:'0.3rem' }}>
                {s.label} →
              </Link>
            </div>
          ))}
