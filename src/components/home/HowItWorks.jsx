    ...
    <div style={{ background:'var(--gray)', padding:'5rem 2rem' }}>
      <div style={{ maxWidth:1280, margin:'0 auto' }}>
        <div style={{ marginBottom:'2.5rem' }}>
          ...
          <h2 style={{ fontFamily:'Bricolage Grotesque, sans-serif', fontSize:'clamp(1.8rem, 4vw, 2.6rem)', fontWeight:800, color:'var(--dark)', lineHeight:1.15, marginBottom:'0.8rem' }}>
            En 4 étapes <span style={{ color:'var(--primary)' }}>simples</span>
          </h2>
          <p style={{ color:'var(--muted)', fontSize:'1rem', lineHeight:1.7, fontFamily:'system-ui,sans-serif' }}>Que vous soyez passager, expéditeur ou conducteur, démarrez en moins de 5 minutes.</p>
        </div>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit, minmax(220px, 1fr))', gap:'1.5rem' }}>
          {steps.map(s => (
            <div key={s.num}
              style={{ background:'white', borderRadius:16, padding:'2rem 1.6rem', border:'1px solid var(--border)', transition:'all 0.25s' }}
              ...
              <p style={{ color:'var(--muted)', fontSize:'0.88rem', lineHeight:1.65, fontFamily:'system-ui,sans-serif' }}>{s.desc}</p>
            </div>
          ))}
        </div>
      </div>
